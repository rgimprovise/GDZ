#!/usr/bin/env python3
"""
Локальный скрипт (не для VPS): извлечение встроенного текста из PDF без БД и без Tesseract.
Пишет data/ocr_raw/{book_id}/{source_id}_embedded.md и data/ocr_normalized/{book_id}/{source_id}.md.
Дальше можно подгрузить 1.md на VPS и запустить «LLM нормализация» / «Распределение в БД» из интерфейса.

Использование (из корня репозитория):
  python scripts/local_only/run_embedded_pdf_extract.py --pdf "data/pdfs/...-ocr.pdf" --book-id 1 --source-id 1
  python scripts/local_only/run_embedded_pdf_extract.py   # ищет *ocr*.pdf в data/pdfs, book_id=1, source_id=1
"""
import argparse
import sys
from pathlib import Path

# Корень репозитория (скрипт лежит в scripts/local_only/)
...
def main():
    ...
    # Пишем в формате ocr_files (без зависимости от БД)
    base = data_dir
    ...
    print("Дальше: подгрузите .md на VPS (см. docs/UPLOAD_NORMALIZED_TO_VPS.md) и запустите «LLM нормализация» / «Распределение в БД».")
