...
- Титул, выходные данные: характерные фразы (издательство, ISBN, тираж, «©», «М.: Просвещение», «Учебник для … классов»). Отмечай как other.
- Теоретический материал параграфа: связный текст с определениями, формулировками, доказательствами; нумерация «N. Подзаголовок» внутри параграфа — это пункт теории (subsection), не задача. Отмечай как section_theory или theory.
- Задачи: явные пометки «Задача (N)», «Упражнение N», «№ N» или текст условия после заголовка «Задачи к параграфу». Короткая строка «N. Название» без условия и без связного текста параграфа — оглавление или пункт параграфа, не problem.
...
- В конце учебника: предметный указатель — список «термин — стр. N» или «термин, стр. N»; блок ответов к задачам — перечень номеров и кратких ответов (ответы не лежат рядом с условиями, их нужно сопоставить с задачами по номеру). Блок ответов размечай как type: "answers_block" и заполняй поле "answers": [{"number": "315", "answer_text": "…"}, …]. Обработчик сопоставит по номеру задачи и запишет answer_text в БД. Предметный указатель — other.
...
Правила: block_id — порядковый номер блока (1, 2, 3, ...). section — номер параграфа из запроса (например §1). theory_text — для теории, problem_text — для задач. solution_only — блок «Решение.» привязать к предыдущей задаче. Не помещай теоретический текст в problem_text."""
