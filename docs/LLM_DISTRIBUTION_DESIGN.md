...
**Общее правило:** в промптах и скриптах не использовать частные примеры из конкретного учебника (предмет, верстка различаются). Если пример или паттерн нужен — явно указывать, откуда он взят и для какого случая действует (например: «типичные для российских учебников», «при верстке с § N»).
...
**Нумерованные подзаголовки теории vs задачи**: строка вида «N. Заголовок» (нумерованный подпункт внутри параграфа) — подпункт теории, не задача. В промпте явно указано не помещать такие блоки в type: problem. В препроцессинге (режим по блокам) короткие строки «N. Заголовок» без ключевых слов формулировки задачи (докажите, найдите, упражнение, задача… — типичные для рус. учебников) не режутся как граница задачи и остаются в блоке теории.

**Страницы**: в режиме по параграфам всем блокам из одного параграфа присваивается страница начала параграфа (в нормализованном файле нумерация страниц 1, 2, 3, …). Для более точной привязки к странице используется режим по блокам (каждый блок получает страницу по месту в тексте).
