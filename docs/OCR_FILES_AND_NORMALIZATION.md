...
|------|-------------|--------|
| **OCR (сырой результат)** | **Файлы** (например `.md` в каталоге `data/ocr_raw/`) | Сравнение Tesseract / другой модели; чтение и проверка человеком; один раз распознали — дальше работаем с файлом. |
...
Внутри — только текст одной страницы (при необходимости с минимальным frontmatter: `page_num`, `model`). Удобно для сравнения двух OCR по одной и той же странице (два файла с суффиксом модели или в подкаталогах `tesseract/`, и т.п.).
...
- **Реализовано (ingestion + ocr_files):**
  1. OCR (Tesseract) по всем страницам → запись в **файл** `data/ocr_raw/{book_id}/{pdf_source_id}_{model}.md` (формат .md по страницам: `## Страница N`).
